...
const ScrollStack = ({
  ...
  const geometryRef = useRef({ cardTops: [], endTop: 0 });
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    const { cardTops, endTop: endElementTop } = geometryRef.current;
    let topCardIndex = 0;
    if (blurAmount) {
      cardTops.forEach((cardTop, index) => {
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      const cardTop = cardTops[i] ?? getElementOffset(card);
  ...
  const setupLenis = useCallback(() => {
    const startLoop = lenis => {
      ...
      if (!animationFrameRef.current && isVisibleRef.current && !document.hidden) {
        animationFrameRef.current = requestAnimationFrame(raf);
      }
    };

    if (lenisRef.current) {
      startLoop(lenisRef.current);
  ...
  useLayoutEffect(() => {
    ...
    const measureGeometry = () => {
      ...
      geometryRef.current = {
        cardTops: cards.map(getElementOffset),
